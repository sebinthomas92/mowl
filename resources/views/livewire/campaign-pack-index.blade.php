<div class="app-shell" x-data="{ mobileNav: false }">
    ...
    <main class="main-panel" id="main-content">
        ...
        <section class="library-page packs-library">
            ...
            <div class="stat-strip"><div><small>PACKS CREATED</small><strong>{{ $packs->count() }}</strong></div><div><small>CREDITS REMAINING</small><strong>{{ $workspace->creditBalance() }}</strong></div><div><small>COGS STATUS</small><strong @class(['healthy' => $packs->every(fn($pack) => $pack->estimated_cost < config('campaigns.cogs_alert'))])>{{ $packs->contains(fn($pack) => $pack->estimated_cost >= config('campaigns.cogs_alert')) ? 'Needs attention' : 'On target' }}</strong></div></div>
            @if($packs->isEmpty())
            ...
            @else
                <div class="pack-list">@foreach($packs as $pack)<a href="{{ route('campaign-packs.show', $pack) }}" class="pack-row"><div class="pack-row-mark">{{ strtoupper(substr($pack->product->name, 0, 2)) }}</div><div class="pack-row-main"><small>{{ $pack->product->brand->name }}</small><h2>{{ $pack->product->name }}</h2><p>{{ $pack->current_version ? 'Campaign Pack v'.$pack->current_version : 'Generation job #'.$pack->latestGenerationJob?->id }} · Updated {{ $pack->updated_at->diffForHumans() }}</p></div><span @class(['approved-badge', 'job-status-failed' => $pack->status === 'failed', 'job-status-working' => in_array($pack->status, ['queued','processing'])])>{{ $pack->status === 'approved' ? '✓' : '●' }} {{ ucfirst($pack->status) }}</span><strong>Open {{ $pack->status === 'approved' ? 'pack' : 'job' }} →</strong></a>@endforeach</div>
            @endif
        </section>
    </main>
</div>
