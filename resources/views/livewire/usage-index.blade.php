    ...
    <main class="main-panel" id="main-content">
        ...
        <section class="library-page usage-page">
            ...
            <div class="usage-metrics">
                ...
                <article><span>COMPLETED JOBS</span><strong>{{ $completedJobs }}</strong><small>Pack, section, and banner generations</small></article>
            ...
            <div class="usage-layout">
                <section class="usage-ledger" aria-labelledby="jobs-heading">
                    <div class="team-section-heading"><div><p class="kicker">PROVIDER LEDGER</p><h2 id="jobs-heading">Generation jobs</h2></div><span>{{ $jobs->count() + $bannerBatches->count() }} recent</span></div>
                    @if($jobs->isEmpty() && $bannerBatches->isEmpty())
                    ...
                    @else
                        ...
                        @endforeach
                        @foreach($bannerBatches as $batch)
                            <article class="job-ledger-row">
                                <div><strong>{{ $batch->campaignPack?->product?->name ?? 'Campaign pack' }}</strong><small>Banner studio · {{ $batch->kind }} · {{ $batch->requested_count }} {{ Str::plural('image', $batch->requested_count) }} · {{ $batch->created_at->format('M j, H:i') }}</small></div>
                                <span @class(['ledger-status', 'failed' => in_array($batch->status, ['failed', 'partial'])])>{{ strtoupper($batch->status) }}</span>
                                <span>{{ number_format(($batch->input_tokens + $batch->output_text_tokens + $batch->output_image_tokens) / 1000, 1) }}K <small>tokens · {{ $batch->provider ?: 'pending' }}</small></span>
                                <strong @class(['ledger-cost', 'alert' => $batch->cost_alert])>${{ number_format((float) $batch->estimated_cost, 3) }} @if($batch->cost_alert)<i>!</i>@endif</strong>
                            </article>
                        @endforeach
                    @endif
                </section>
